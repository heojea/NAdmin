...
@Service
public class MobileCloseAccountService {
     ...
     * @throws Exception
     */
	public DataMap selectMobileCloseJson(HttpServletRequest request) throws Exception {
		...
		dataMap = mobileCloseAccountDao.selectMobileCloseDaoListCnt(param);
		int totalCnt = dataMap.getInt("TOT_CNT");
		
		param.put("totalCnt",totalCnt);
		
        list.add(dataMap);
		list.addAll(mobileCloseAccountDao.selectMobileCloseDaoList(param));
	 ...
	 * @throws Exception
	 */
	public void selectMobileCloseBigData(DataMap param ,  RowHandler handler) throws Exception{
		mobileCloseAccountDao.selectMobileCloseDaoListBigData(param, handler);
	}
}
